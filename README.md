...
2. **Session memory** — after every agent stop **where files were changed**, a hook captures what changed and creates `sessions/YYYY-MM-DD-N.md`. You append caveman-style "why" bullets. `memory.md` is the index. Pure Q&A sessions with no file changes produce no session file.
...
| `.cursor/hooks.json` | Hook config — fires `on-session-end.sh` on agent stop |
| `.cursor/hooks/on-session-end.sh` | Loop guard → detect changes → create session file → update memory.md → graphify sync |
...
The hook fires at the end of every Cursor agent stop. It first checks `git diff` (staged + unstaged) for changed files. **If no files changed, it exits silently — no session file is created.** This means pure Q&A conversations don't generate session files, only sessions where the agent actually modified something.
...
1. Hook creates `sessions/2026-06-17-1.md` (date + incrementing number per day) with frontmatter:
   ...
   time: 14:32
   session: 1
   topics: "hooks/on-session-end.sh"
   files_changed: 3
   files:
     - src/auth.ts
     - src/db.ts
   ...
   ---
   ```
2. Cursor injects a follow-up message prompting the agent to fill in three sections:
   ```markdown
   ## What happened
...
> **Note:** The hook also guards against loops — it only fires on the first stop of a chat (not on follow-up turns), and skips if a session file already has decisions written.
...
|---------|-----------|------|
| Agent stop (files changed) | Incremental AST update + god node sync into `main.mdc` | No |
